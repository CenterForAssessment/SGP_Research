...
if (tolower(params$render.format) == "pagedreport") {
	#  Too long of a quote, but here's how to include calculations/R code within the quote:
	# Correlations between one-year SGP and skip-year SGP shown in Table", tblNum(1), "by content area and grade range from", round(min(indiv_smry$SGP_Corr, na.rm=TRUE), 2), "to", round(max(indiv_smry$SGP_Corr, na.rm=TRUE), 2), ".
cat(
"<aside>

> This indicates that although the quantities are for different time spans, students showing high or low growth across one of the time spans often showed a similar trend across the other time span.

<p class='quote-author'>-- Damian Betebenner</p>
...
```{r, results='asis', echo=FALSE}
	...
	tmp.caption <- 'Sequential and skip-year SGP correlation, and SGP mean/standard deviation and test score correlations by content area and grade.'
	knitr::knit_print(htmlTable::htmlTable(SGP_Corrs, title="",
	  align="rrccccccc",
	  n.cgroup=c(3, 3, 3), cgroup=c("", "Sequential", "Skip Year"),
