...
```{r, cache=FALSE, results="asis", echo=FALSE}
	...
	for (ca in params$GoF_GL_subjects) {
		...
		# wmwd2 <- knitr::knit_print(print(getwd())) # ...Documentation/assets/rmd/NCIEA_Report
		...
		seq.tmp.dir <- lapply(file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", gsub(params$gof.path.seq, "", seq.plot.list, fixed=TRUE)), strsplit, "/")
		for (d in seq(seq.tmp.dir)) { # should all be the same here (same content area) and already created in seq only
			if(!file.exists(file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", tail(seq.tmp.dir[[d]][[1]], 2)[1]))) {
				dir.create(file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", tail(seq.tmp.dir[[d]][[1]], 2)[1]))
			}
		}
		doc.seq.plot.paths <- file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", gsub(params$gof.path.seq, "", seq.plot.list, fixed=TRUE))
		file.copy(seq.plot.list, doc.seq.plot.paths)

		skip.tmp.dir <- lapply(file.path("..", "assets", "img", "Goodness_of_Fit", "Skip_Year", gsub(params$gof.path.skip, "", skip.plot.list, fixed=TRUE)), strsplit, "/")
		for (d in seq(skip.tmp.dir)) { # should be needed ONCE
			if(fe.tf <- !file.exists(file.path("..", "assets", "img", "Goodness_of_Fit", "Skip_Year", tail(skip.tmp.dir[[d]][[1]], 2)[1]))) {
				dir.create(file.path("..", "assets", "img", "Goodness_of_Fit", "Skip_Year", tail(skip.tmp.dir[[d]][[1]], 2)[1]), recursive = TRUE)
			} # fe.tf to trap TRUE/FALSE output
		}
		doc.skip.plot.paths <- file.path("..", "assets", "img", "Goodness_of_Fit", "Skip_Year", gsub(params$gof.path.skip, "", skip.plot.list, fixed=TRUE))
		fc.tf <- file.copy(skip.plot.list, doc.skip.plot.paths)
...
## ", capwords(GOF_Subj), "
		...
		for (k in seq(plot.chunks)){
			placeFigure(
					files = c(rbind(doc.seq.plot.paths[plot.chunks[[k]]], doc.skip.plot.paths[plot.chunks[[k]]])),
