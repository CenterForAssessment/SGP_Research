...
```{r, cache=FALSE, results="asis", echo=FALSE}
	...
	seq.tmp.dir <- lapply(file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", gsub(params$gof.path.seq, "", seq.only.plots, fixed=TRUE)), strsplit, "/")
	for (d in seq(seq.tmp.dir)) {
		if(fe.tf <- !file.exists(file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", tail(seq.tmp.dir[[d]][[1]], 2)[1]))) {
			dir.create(file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", tail(seq.tmp.dir[[d]][[1]], 2)[1]), recursive = TRUE)
		} # fe.tf to trap TRUE/FALSE output
	}
	documentation.plot.paths <- file.path("..", "assets", "img", "Goodness_of_Fit", "Sequential", gsub(params$gof.path.seq, "", seq.only.plots, fixed=TRUE))
	fc.tf <- file.copy(seq.only.plots, documentation.plot.paths)

	placeFigure(
		files = documentation.plot.paths,
