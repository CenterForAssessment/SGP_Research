...
```{r, cache=FALSE, results="asis", echo=FALSE}
	...
	if (is.null(params$skip.grades.list)) {
		for (ca in params$GL_subjects) seq.skip.grd[[ca]] <- intersect(params$seq.grades, params$skip.grades)
	} else {
		for (ca in params$GL_subjects) seq.skip.grd[[ca]] <- intersect(params$seq.grades.list[[ca]] %w/o% "EOCT", params$skip.grades.list[[ca]] %w/o% "EOCT")
	}
	seq.skip.grd <- sapply(sapply(seq.skip.grd, as.numeric), sort)  #  make numeric for re-order when grades >= 10 present

	for (ca in params$GL_subjects) {
		seq.plot.list <- NULL
		skip.plot.list <- NULL

		seq.plots <-  unique(gsub(".png|.pdf", "", list.files(file.path(params$gof.path.seq[[1]], paste0(ca, ".2019", gof.type)))))
		...
		seq.plot.list <- c(seq.plot.list, file.path(params$gof.path.seq[[1]], paste0(ca, ".2019", gof.type), paste0(seq.plots[!is.na(seq.plots)], ".pdf")))

		skip.plots <-  unique(gsub(".png|.pdf", "", list.files(file.path(params$gof.path.skip[[1]], paste0(ca, ".2019", gof.type)))))
		...
		skip.plot.list <- c(skip.plot.list, file.path(params$gof.path.skip[[1]], paste0(ca, ".2019", gof.type), paste0(skip.plots[!is.na(skip.plots)], ".pdf")))
