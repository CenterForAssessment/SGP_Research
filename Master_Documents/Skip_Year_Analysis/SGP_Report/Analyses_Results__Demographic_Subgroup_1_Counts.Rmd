...
###   Calculate Indiv/Group level values
	demog_smry <- Report_Data[VALID_CASE=="VALID_CASE" & GRADE %in% params$skip.grades & !is.na(eval(parse(text = demog.group$variable))), list(
                  SGP_Correlation = round(cor(NO_SKIP_SGP, SGP, use="pairwise.complete"), 2),
									...
									Skip_Count = sum(!is.na(SGP))), keyby = c("CONTENT_AREA", demog.group$variable)]
...
```{r, results='asis', echo=FALSE}
	demog_counts <- demog_smry[, c(1:2, 8:10, 12:13)] # Percentages receiving SGPs
	demog_counts$CONTENT_AREA <- sapply(demog_counts$CONTENT_AREA, capwords, USE.NAMES=FALSE)
	demog_counts$CONTENT_AREA[duplicated(demog_counts$CONTENT_AREA)] <- ""
	...
	setcolorder(demog_counts, c(1:4, 6, 5, 7))
	...
	cat(dualTable(as.matrix(demog_counts),
  	align=paste(rep("r", ncol(demog_counts)), collapse=""),
		...
		caption= paste("Sequential and skip-year SGP counts and percentages by content area and",  demog.group$label, "status")))

#   dualTable(as.matrix(demog_counts),
#   	align=paste(rep("r", ncol(demog_counts)), collapse=""),
# 		n.cgroup=c(3, 2, 2), cgroup=c("", "Sequential", "Skip Year"),
# 		caption= paste("Sequential and skip-year SGP counts and percentages by content area and",  demog.group$label, "status"),
# 		file=file.path(params$working.directory, "assets", "tables", paste0("Table_", tblNum(), ".txt")))
#
# 	cat.text <- paste("```{r, results='asis', echo=FALSE}\ncat(rmarkdown:::read_utf8(file.path('assets', 'tables', ", paste0("'Table_", tblNum(), ".txt'"), ")), sep = '\\n')\n```")
#   cat(
# cat.text)


	# cat(c("\n\n", readLines(file.path(params$working.directory, "assets", "tables", paste0("Table_", tblNum(), ".txt")))), sep = "\n")

	# cat.text <- paste("```{r AR_1, child = file.path(params$working.directory,", "'assets', 'tables',", paste0("'Table_", tblNum(), ".txt'),"), "eval=TRUE}\n```")
#   cat(
# cat.text)

	# cat(c('\n\n', readLines(file.path(params$working.directory, 'assets', 'tables',  'Table_0.txt' ))), sep = '\n')
	# cat(rmarkdown:::read_utf8(file.path('assets', 'tables',  'Table_0.txt' )), sep = '\n')
