...
```{r, cache=FALSE, echo=FALSE}
	...
	for (ca in params$EOC_subjects) {
		...
		canonical.year <- c("2019", SGP:::yearIncrement("2019", 1, tail(cumsum(head(rev(
		...
		seq.plot.list <- file.path(params$gof.path.seq[[1]], paste0(ca, ".2019", gof.type), paste0(canonical.prog[which.max(nchar(canonical.prog))], ".pdf")) # which.max to only do the full progression plot
		...
		###   Skip Year Analyses
		skp.plots <-  unique(gsub(".png|.pdf", "", list.files(file.path(params$gof.path.skip[[1]], paste0(ca, ".2019", gof.type)))))
		...
		skp.plot.list <- file.path(params$gof.path.skip[[1]], paste0(ca, ".2019", gof.type), paste0(canonical.prog.seq, ".pdf")) # which.max to only do the full progression plot
