...
```{r, echo=FALSE, include=FALSE}
   ...
   p <- p + geom_point(data = demog_plot_data_gl[!is.na(eval(parse(text = demog.group$variable)))],
   										aes(x = eval(parse(text = demog.group$school.variable)), y = Mean_Difference, size = COUNT_NO_SKIP_SGP), alpha = 0.25, colour="blue") +
            scale_size_continuous(range = c(1, 9))
   p <- p + geom_smooth(data = demog_plot_data_gl[!is.na(eval(parse(text = demog.group$variable)))],
