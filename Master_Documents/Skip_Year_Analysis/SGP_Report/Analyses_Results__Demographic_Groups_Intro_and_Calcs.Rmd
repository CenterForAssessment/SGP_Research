...
###   Create data set for plots
	demog_plot_data_gl <- rbindlist(list(
		sch_demog_msgp[COUNT_NO_SKIP_SGP >= params$min.school.size & COUNT_SKIP_SGP >= params$min.school.size & CONTENT_AREA %in% params$GL_subjects][, Schools := "All Schools"],
    sch_demog_msgp_x[COUNT_NO_SKIP_SGP >= params$min.school.size & COUNT_SKIP_SGP >= params$min.school.size & CONTENT_AREA %in% params$GL_subjects][, Schools := "Skip-Year Subset"]))
  ...
  cor_demog <- demog_plot_data_gl[COUNT_NO_SKIP_SGP >= params$min.school.size & COUNT_SKIP_SGP >= params$min.school.size & !is.na(MEAN_SKIP_SGP),
  																	lapply(smry_eval_expression, function(f) eval(parse(text=f))), keyby=c("CONTENT_AREA", "Schools")]

	##  EOC Subjects
	if (!is.null(params$EOC_subjects)) {
	  demog_plot_data_eoc <- sch_demog_msgp[COUNT_NO_SKIP_SGP >= params$min.school.size & COUNT_SKIP_SGP >= params$min.school.size & CONTENT_AREA %in% params$EOC_subjects]
	  ...
	  cor_demog_eoc <- demog_plot_data_eoc[COUNT_NO_SKIP_SGP >= params$min.school.size & COUNT_SKIP_SGP >= params$min.school.size & !is.na(MEAN_SKIP_SGP),
	  																	lapply(smry_eval_expression, function(f) eval(parse(text=f))), keyby="CONTENT_AREA"]
	}
```
