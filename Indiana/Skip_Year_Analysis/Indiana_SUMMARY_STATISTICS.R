...
print(Indiana_SGP_LONG_Data[GRADE %in% as.character(4:8) & VALID_CASE=="VALID_CASE", list(MEAN_NO_SKIP_SGP=mean(NO_SKIP, na.rm=TRUE), SD_NO_SKIP_SGP=sd(NO_SKIP, na.rm=TRUE)), key=c("CONTENT_AREA", "GRADE")])
print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(MEAN_SKIP_SGP=mean(SGP, na.rm=TRUE), SD_SKIP_SGP=sd(SGP, na.rm=TRUE)), key=c("CONTENT_AREA", "GRADE")])


### Correlation between one-year and skip-year SGPs

print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(CORRELATION_SGP_SKIP_YEAR_SGP=cor(SGP, NO_SKIP, use="complete.obs")), key=c("CONTENT_AREA", "GRADE")])
print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(CORRELATION_SGP_SKIP_YEAR_SGP=cor(SGP, NO_SKIP, use="complete.obs")), key=c("CONTENT_AREA", "ETHNICITY")])
print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(CORRELATION_SGP_SKIP_YEAR_SGP=cor(SGP, NO_SKIP, use="complete.obs")), key=c("CONTENT_AREA", "FREE_REDUCED_LUNCH_STATUS")])
print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(CORRELATION_SGP_SKIP_YEAR_SGP=cor(SGP, NO_SKIP, use="complete.obs")), key=c("CONTENT_AREA", "IEP_STATUS")])

### Differences between one-year and skip-year SGPs

print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(MEAN_ABSOLUTE_SGP_DIFFERENCE=mean(abs(SGP - NO_SKIP), na.rm=TRUE)), key=c("CONTENT_AREA", "GRADE")])
print(quantile(Indiana_SGP_LONG_Data$SGP - Indiana_SGP_LONG_Data$NO_SKIP, probs=1:9/10, na.rm=TRUE))

### Percentage of students with one-year SGP

print(Indiana_SGP_LONG_Data[GRADE %in% as.character(4:8) & VALID_CASE=="VALID_CASE", list(PERCENTAGE_with_SGP=100*sum(!is.na(NO_SKIP))/.N), key=c("CONTENT_AREA", "GRADE")])

### Percentage of students with skip-year SGP

print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(PERCENTAGE_with_SGP=100*sum(!is.na(SGP))/.N), key=c("CONTENT_AREA", "GRADE")])

### Percentage of students with one-year but no skip-year SGP

print(Indiana_SGP_LONG_Data[GRADE %in% as.character(5:8) & VALID_CASE=="VALID_CASE", list(PERCENTAGE_NO_SKIP_wout_SKIP=100*sum(!is.na(SGP))/sum(!is.na(NO_SKIP))), key=c("CONTENT_AREA", "GRADE")])

### Percentage of students with skip-year SGP but no one-year SGP

print(Indiana_SGP_LONG_Data[GRADE %in% as.character(4:8) & VALID_CASE=="VALID_CASE", list(PERCENTAGE_SKIP_wout_NO_SKIP=100*sum(!is.na(SGP))/sum(is.na(NO_SKIP))), key=c("CONTENT_AREA", "GRADE")])
...
Missing.skip.year.SGP.students <- Indiana_SGP_LONG_Data[is.na(SGP) & !is.na(NO_SKIP) & GRADE!="4" & CONTENT_AREA %in% c("ELA", "MATHEMATICS")]

### ETHNICITY analysis

print(prop.table(table(Indiana_SGP_LONG_Data$GRADE, Indiana_SGP_LONG_Data$ETHNICITY), margin=1))
...
print(prop.table(table(Indiana_SGP_LONG_Data$GRADE, Indiana_SGP_LONG_Data$FREE_REDUCED_LUNCH_STATUS), margin=1))
...
print(Indiana_SGP_LONG_Data[VALID_CASE=="VALID_CASE" & CONTENT_AREA %in% c("ELA", "MATHEMATICS"), list(MEAN_PRIOR_STANDARDIZED=mean(SCALE_SCORE_NO_SKIP_PRIOR_STANDARDIZED, na.rm=TRUE)), by=c("CONTENT_AREA", "GRADE")])
...
print(Indiana_SGP_LONG_Data[VALID_CASE=="VALID_CASE" & CONTENT_AREA %in% c("ELA", "MATHEMATICS"), list(MEAN_SGP=mean(NO_SKIP, na.rm=TRUE)), by=c("CONTENT_AREA", "GRADE")])
...
Indiana.school.summaries <- Indiana_SGP_LONG_Data[,
                        list(
...
Indiana.school.EMH.correlations <- Indiana.school.summaries[COUNT_NO_SKIP_SGP>=10 & COUNT_SKIP_SGP>=10,
                        list(COR_SKIP_NO_SKIP_MEAN_SGP=cor(MEAN_NO_SKIP_SGP, MEAN_SKIP_SGP, use="complete.obs")),
                        keyby=c("EMH_LEVEL", "CONTENT_AREA")]
print(Indiana.school.EMH.correlations)
