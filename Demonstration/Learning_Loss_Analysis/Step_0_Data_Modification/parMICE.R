parMICE <- function(
    ...
    packages=NULL, cluster.type=NULL, ...){ # mice.fun = NULL

  if (is.null(cluster.type)) cluster.type <- "PSOCK"

  cl <- makeCluster(nnodes, type = cluster.type) # , setup_strategy = "sequential" # Error when running SGP in parallel before this is used...
