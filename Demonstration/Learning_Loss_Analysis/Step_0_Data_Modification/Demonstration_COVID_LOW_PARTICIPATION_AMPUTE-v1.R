...
if (exists("custom.ampSS.arg.list")) {
  variables.to.get <- c("VALID_CASE", "ID", "YEAR", unique(c(
      custom.ampSS.arg.list$default.vars %w/o% c("SCALE_SCORE_COMPLETE", "ACH_LEV_COMPLETE"), # "ACTUAL" vars created below
...
setnames(Demonstration_COVID_SGP@Data, c("SGP", "SGP_BASELINE"), c("SGP_COMPLETE", "SGP_BASELINE_COMPLETE"))
setnames(Demonstration_COVID_SGP_LONG_Data_2021, c("SGP", "SGP_BASELINE"), c("SGP_COMPLETE", "SGP_BASELINE_COMPLETE"))
...
Pre_COVID_Data <- rbindlist(
    list(Demonstration_COVID_SGP@Data[YEAR %in% 2018:2019, variables.to.get, with=FALSE],
         Demonstration_COVID_SGP_LONG_Data_2021[, variables.to.get, with=FALSE]))[,
                SCALE_SCORE_COMPLETE := SCALE_SCORE][, # Create "ACTUAL" vars
